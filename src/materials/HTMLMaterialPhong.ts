...
void main() {
	...
	gl_FragColor = vec4( outgoingLight, reflectedLight.directSpecular * roughness);
	
	#ifdef USE_FOG

		gl_FragColor = vec4(gl_FragColor.rgb, gl_FragColor.a * (1.0 - fogFactor));
...
export class HTMLMaterialPhong extends HTMLShaderMaterial {
	...
	set specular(value: THREE.Color) {
			...
			uniforms: THREE.UniformsUtils.merge(
				[
					...
					THREE.UniformsLib[ "fog" ],
					{
						...
						specular: { value: new THREE.Color(1, 1, 1) },
						shininess: { value: 30 },
						roughness: { value: 1 },
					},
				]
