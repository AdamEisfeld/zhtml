...
type DemoType = string & ('persp' | 'ortho' | 'stereo');

export default defineComponent({
  name: 'DemoApplication',
  components: { HTMLRenderViewSplit },
  setup() {
    const demo_type: DemoType = 'stereo';
    const show_debug_quad = true;
    ...
    onMounted(() => {
      ...
      const controls = new OrbitControls(camera, scene_container_element);
...
<template>
  <div class="w-full h-full flex flex-col items-center m-auto gap-8 bg-slate-800">
    <div class="flex flex-col gap-8 m-auto w-full h-full">
      ...
      >
        <HTMLRenderViewSplit
          :is_split_enabled="is_stereo()"
          class="absolute left-0 top-0 w-full h-full"
          :render_target="render_targets[0]"
          :left_render_target="render_targets[0]"
          :right_render_target="render_targets[1]"
